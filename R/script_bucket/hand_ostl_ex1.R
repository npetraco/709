...
tc1l   <- qt(alp, df = n-1) # critical t-statistic for H0:mu>=50 vs. Ha:mu<50
...
points(x = tc1l, y = 0, pch=16, col="green") # Min where we'd expect sample to fall IF (H0) mu>=50
points(x = t.samp, y = 0, pch=16, col="red") # Where sample ACTUALLY falls
pt(q = t.samp, df = n-1)                     # one-sided p-value (i.e. for H0:mu>=50 vs. Ha:mu<50)


# Well actually, you can do all this in one line:
t.test(x, alternative = "less", mu = 50.0000, conf.level = 0.95)
